# ##############################################################################
#
## Create simulations with the method from Weiss et al.
#
# ##############################################################################
...
for (i in names(dataset.list)){
  simulation <- paste0(temp.loc, 'sim_', i, '_Weiss.h5')
  message(simulation)
  if (file.exists(here('simulations', 'others', paste0('sim_', i, '_Weiss.h5')))){next()}
